...
| `jien/` | Pure-Rust port of the P/ECE sample app `jien` (bouncing sprite) — validates the `pceapi` graphics bindings. Its LCD output is byte-identical (frame-hash-equal) to the original C app on piece-emu. |
| `menu2/` | Pure-Rust port of the P/ECE system launcher (the real `startup.pex`): scans PFFS for `.pex`, draws a 3-column icon grid + cursor + caption bubble, and launches apps with `pceAppExecFile`. Boot + navigation output is frame-hash-identical to the original C launcher. |
